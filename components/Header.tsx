...
const Header: React.FC = () => {
  ...
  // Logic to show Driver or Admin links
  if (user?.userType === 'admin') {
      navLinks.push({ path: '/admin/drivers', label: '管理パネル', icon: Shield });
  } else if (user?.userType === 'driver') {
      navLinks.push({ path: '/driver', label: 'ドライバー画面', icon: User });
  } else {
      // For guests or customers, we don't put driver link in main nav to keep it clean, 
      // but maybe a small partner link is fine.
  }
        ...
        <div className="flex justify-between h-20">
          <div className="flex items-center">
            <Link to="/" className="flex-shrink-0 flex items-center gap-3 group">
              <div className="bg-gradient-to-br from-brand-400 to-brand-600 text-white p-2 rounded-xl shadow-lg shadow-brand-500/30 transform group-hover:scale-105 transition-transform duration-300">
                <Truck size={22} />
              </div>
              <div className="font-bold text-2xl tracking-tight group-hover:opacity-90 transition-opacity">
                <span className="text-slate-900">Aoneko</span>
                <span className="text-brand-500">Move</span>
              </div>
            </Link>
          </div>
          ...
          <div className="hidden md:flex md:items-center md:space-x-6">
            ...
            <div className="pl-6 border-l border-slate-200 flex items-center gap-4">
              {user ? (
                ...
                <>
                  <Link to="/login" className="text-slate-500 hover:text-brand-600 font-medium text-sm px-4 transition-colors">
                    ログイン
                  </Link>
                  <Link to="/signup" className="bg-slate-900 text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-brand-600 transition-all shadow-lg hover:shadow-brand-500/30 transform hover:-translate-y-0.5">
                    新規登録
                  </Link>
                  <div className="h-4 w-px bg-slate-200 mx-1"></div>
                  <Link to="/partner/register" className="text-xs font-bold text-slate-400 hover:text-slate-600 uppercase tracking-wide">
                    ドライバー登録
                  </Link>
                </>
              )}
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white/95 backdrop-blur-xl border-t border-slate-100 absolute w-full shadow-2xl animate-fade-in-up">
          <div className="px-4 pt-4 pb-8 space-y-2">
            ...
            {/* Mobile specific partner link if not logged in */}
            {!user && (
                 <Link
                    to="/partner/register"
                    onClick={() => setIsMenuOpen(false)}
