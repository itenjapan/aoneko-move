...
import { Delivery, LatLng, Driver } from '../types';
                      ...
                      availableJobs.map(job => (
                         <div key={job.id} className="bg-white rounded-2xl shadow-sm p-6 animate-fade-in-up border border-slate-100 hover:shadow-md transition-shadow">
                            <div className="flex justify-between items-start mb-4">
                               ...
                               <div className="text-xl font-bold text-slate-900">¥{Math.floor(job.price.total * 0.85).toLocaleString()}</div>
                            </div>
                ...
                {completedJobs.length > 0 ? completedJobs.map(job => (
                    ...
                    >
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <div className="text-right">
                                <p className="text-lg font-bold text-brand-600">¥{Math.floor(job.price.total * 0.85).toLocaleString()}</p>
                        ...
                        {expandedHistoryId === job.id && job.price.breakdown && (
                          <div className="mt-4 pt-4 border-t border-slate-100 bg-slate-50/80 -mx-6 px-6 pb-4 animate-fade-in-up">
                            ...
                            <div className="space-y-2.5 text-xs font-medium">
                              ...
                              <div className="flex justify-between font-bold text-rose-400"><span>プラットフォーム利用料 (15%)</span><span className="font-mono">- ¥{Math.ceil(job.price.total * 0.15).toLocaleString()}</span></div>
                              <div className="pt-2 bg-brand-50 -mx-4 px-4 py-3 rounded-xl flex justify-between font-extrabold text-brand-600 shadow-inner border border-brand-100/50"><span>最終受取報酬</span><span className="font-mono text-xl">¥{Math.floor(job.price.total * 0.85).toLocaleString()}</span></div>
                            </div>
                          </div>
                        )}
