...
const QuickQuote: React.FC = () => {
  ...
  const handleCalculateQuote = async () => {
    ...
    setLoading(true);
    try {
      // Robust LatLng Recovery: If user typed manually instead of selecting from dropdown
      let finalPickupLatLng = pickupLatLng;
      let finalDeliveryLatLng = deliveryLatLng;

      if (!finalPickupLatLng && pickup) {
        try {
          finalPickupLatLng = await GoogleMapsService.geocode(pickup);
          setPickupLatLng(finalPickupLatLng);
        } catch (e) {
          console.warn("Could not geocode pickup address automatically", e);
        }
      }

      if (!finalDeliveryLatLng && delivery) {
        try {
          finalDeliveryLatLng = await GoogleMapsService.geocode(delivery);
          setDeliveryLatLng(finalDeliveryLatLng);
        } catch (e) {
          console.warn("Could not geocode delivery address automatically", e);
        }
      }
      ...
      if (finalPickupLatLng) {
        setNearestDriver({
          lat: finalPickupLatLng.lat + (Math.random() - 0.5) * 0.02,
          lng: finalPickupLatLng.lng + (Math.random() - 0.5) * 0.02
        });
      }
      ...
      if (customerName || customerEmail) {
        try {
          await supabase.from('leads').insert({
            ...
            move_date: date,
            message: `Vehicle: ${selectedVehicle}, Time: ${time}`
          });
        } catch (err) {
      ...
      alert(`見積もり計算エラー: ${error.message || '住所が正しくないか、サービスが利用できません。'}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleBooking = async () => {
    ...
    if (!quote) return;
    if (!pickup || !delivery) {
      alert('住所エラー: Google Mapsから住所を選択してください / Please select valid addresses.');
      return;
    }
    ...
    try {
      ...
      const { data, error } = await supabase.from('orders').insert({
        ...
        vehicle_id: selectedVehicle === 'keivan' ? 1 : 2, // Map to DB IDs (assuming 1=keivan, 2=kaitruck for now or use name)
        // We really should use the ID from the DB if possible, but for MVP:
        // Let's assume 1 and 2 or fetch them. For robustness, let's use the valid IDs if we had them.
        // Earlier I saw IDs 1 (Camioneta Pequeña) and 2 (Minivan).
        // 1=Camioneta Pequeña=Permit 2=Minivan=Permit.
        // 'keivan' is Minivan -> 2. 'keitruck' is Camioneta -> 1.
        base_fare: quote.basePrice,
        highway_toll: quote.tollFee,
        loading_fee: quote.helperFee + quote.cargoSurcharge, // Combine or separate
        waiting_fee: 0,
        net_price: quote.subTotal,
        tax_amount: quote.tax,
        total_customer_price: quote.totalPrice,
        company_revenue: Math.floor(quote.subTotal * 0.2), // 20%
        driver_revenue: Math.floor(quote.subTotal * 0.8),  // 80%
        status: 'confirmed',
      ...
      // 3. Email Trigger (Simulation)
      // In a real app, Supabase Edge Function triggers on INSERT.
      // Here we trust that happens. We can log.
      console.log("Order saved to Supabase:", data);

      // Simulate Email sending log
      console.log(`[Email Service] Sending confirmation to ${customerEmail || user?.email || 'admin'}...`);
      ...
      const newDelivery = mockStore.createDelivery({
        ...
        // We can use the real ID from Supabase for tracking: data.id
      });

      // Update tracking number to match Supabase ID if we wanted, but mockStore uses its own.
      // Let's stick to mockStore object for display for now to not break UI.
      ...
      setBookingProcessing(false);
      alert(`予約処理に失敗しました。もう一度お試しください。(Error: ${e.message})`);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-12 px-4 sm:px-6">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Customer Information (Lead Capture) */}
        <div className="bg-white rounded-[2.5rem] shadow-sm p-8 md:p-10 border border-slate-100/60">
          <h2 className="text-xl font-bold mb-8 flex items-center text-slate-800">
            <span className="flex items-center justify-center w-10 h-10 rounded-2xl bg-blue-50 text-blue-600 mr-4 ring-4 ring-blue-50/50 shadow-sm">
              <Star size={22} />
            </span>
            お客様情報 (任意)
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="山田 太郎" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:bg-white focus:ring-4 focus:ring-brand-50 transition-all font-medium" />
            </div>
            <div>
              <label className="block text-[10px] font-bold text-slate-400 mb-2 uppercase tracking-widest pl-1">メールアドレス (Email)</label>
              <input type="email" value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)} placeholder="example@email.com" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:bg-white focus:ring-4 focus:ring-brand-50 transition-all font-medium" />
            </div>
            <div className="md:col-span-2">
              <label className="block text-[10px] font-bold text-slate-400 mb-2 uppercase tracking-widest pl-1">電話番号 (Phone) - 任意</label>
              <input type="tel" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} placeholder="090-1234-5678" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:bg-white focus:ring-4 focus:ring-brand-50 transition-all font-medium" />
            </div>
          </div>
          <p className="text-xs text-slate-400 mt-4 ml-1">
            ※ ご入力いただくと、見積もり結果をメールでお送りできます。
          </p>
        </div>
